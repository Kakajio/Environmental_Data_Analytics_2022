...
> Answer: Ho: All Ozone levels are below the threshold for "good" AQI index. H0: mu<=50; Ha:mu>50
...
density<- 
  ggplot(EPAair, 
         aes(x=Ozone, fill=as.factor(Year))) + 
...
pmozone <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(pmozone)
...
EPANUM <- 
  ...
  na.omit()
ex5 <- cor(EPANUM)
corrplot(ex5, method = "ellipse")
corrplot.mixed(ex5, upper = "ellipse")
...
ex6TPAIC <- lm(data = EPA.naomit, PM2.5 ~ Ozone + meanLat + 
              meanLng + Month + Year)
...
EX7<-lm(data = EPA.naomit, PM2.5 ~ Ozone + meanLat + 
              meanLng + Month)
summary(EX7)
```
