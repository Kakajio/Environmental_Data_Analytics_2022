...
```{r}
vector_high <- c(49,46,45,59,66,59,51,51,43,34)
vector_low <- c(31,29,22,27,48,49,33,25,26,15)
```
...
```{r}
celsius_fahrenheit <- function(x){
  celsius <- x/1.5-32/1.5
  return(celsius)
}

celsiushigh <- celsius_fahrenheit(vector_high); celsiushigh
```

```{r}
celsiuslow <- celsius_fahrenheit(vector_low); celsiuslow
```
...
#there are other similar functions ydm(), mdy(), md() etc
```
