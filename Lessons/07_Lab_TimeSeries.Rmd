...
BRA_complete$Date <- as.Date(BRA_complete$Date, format = "%Y-%m-%d")

Rainfallp<- ggplot(BRA_complete, 
         aes(x = Date, y = Rainfall)) + 
  geom_line() +
  geom_smooth(method="lm", color="red")
print(Rainfallp)

Temp1<- ggplot(BRA_complete, 
...
Rain2 <- ts(BRA_complete$Rainfall, start = c(1901,01), frequency = 12)

Temp2 <- ts(BRA_complete$Temperature_C, start = c(1901,01), frequency = 12)
```
...
Rainde <- stl(Rain2, s.window = "periodic")
Tempde <- stl(Temp2, s.window = "periodic")
...
Rainco <- as.data.frame(Rainde$time.series[,1:3])
Tempco <- as.data.frame(Tempde$time.series[,1:3])

Rainco <- mutate(Rainco,
        Observed = BRA_complete$Rainfall,     
        Date =BRA_complete$Date)


Tempco <- mutate(Tempco,
