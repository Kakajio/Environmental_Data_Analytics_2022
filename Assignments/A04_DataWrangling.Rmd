...
```{r, message = FALSE}
#2.1 2019 PM2.5 dataset
dim(PM25_2019)
colnames(PM25_2019)
str(PM25_2019)
#2.2 2018 pm2.5 dataset
dim(PM25_2018)
colnames(PM25_2018)
str(PM25_2018)
#2.3 2019 O3 dataset
...
O3_2019_select <- select(O3_2019, Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE)
O3_2018_select <- select(O3_2018, Date, DAILY_AQI_VALUE, Site.Name, AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, SITE_LONGITUDE)
```
...
write.csv(O3_2019_select, row.names = FALSE, file = "../Data/Processed/EPAair_O3_NC2019_processed.csv")
write.csv(O3_2018_select, row.names = FALSE, file = "../Data/Processed/EPAair_O3_NC2018_processed.csv")
```
...
```{r}
#7
df_four <- rbind(PM25_2019_select, PM25_2018_select, O3_2019_select, O3_2018_select)
df_four$Date <- as.Date(df_four$Date, format = "%m/%d/%Y")
```
...
df_four_processed <- 
  ...
  summarise(meantAQI = mean(DAILY_AQI_VALUE), 
...
```{r}
#9
df_four_spread <- pivot_wider(df_four_processed, names_from = AQS_PARAMETER_DESC, values_from = meantAQI)
```
...
dim(df_four_spread)
#11
write.csv(df_four_spread, row.names = FALSE, file = "../Data/Processed/EPAair_O3_PM25_NC2122_Processed.csv")
```
...
summary_data <-
  ...
  summarise(meanPM = mean(PM2.5), 
            meanOz = mean(Ozone)) %>%
  filter(!is.na(meanPM) & !is.na(meanOz))
...
> Answer: drop_na is to drop the missing data, whereas na.omit is to omit the missing data when calculating, but the missing data are still in the dataset. 
